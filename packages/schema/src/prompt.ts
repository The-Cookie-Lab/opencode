...
export interface ReferenceAttachment extends Schema.Schema.Type<typeof ReferenceAttachment> {}
export const ReferenceAttachment = Schema.Struct({
  name: Schema.String,
  kind: Schema.Literals(["local", "git", "invalid"]),
  uri: Schema.String.pipe(optional),
  repository: Schema.String.pipe(optional),
  branch: Schema.String.pipe(optional),
  target: Schema.String.pipe(optional),
  targetUri: Schema.String.pipe(optional),
  problem: Schema.String.pipe(optional),
  source: Source.pipe(optional),
}).annotate({ identifier: "Prompt.ReferenceAttachment" })
...
export const Prompt = Schema.Struct({
  ...
  references: Schema.Array(ReferenceAttachment).pipe(optional),
})
  .annotate({ identifier: "Prompt" })
  .pipe(
    statics((schema) => ({
      equivalence: Schema.toEquivalence(schema),
      fromUserMessage: (input: Pick<Prompt, "text" | "files" | "agents" | "references">) =>
        schema.make({
          text: input.text,
