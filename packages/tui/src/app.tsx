...
import { isPrintLogsEnabled, logDebug, logError } from "./util/logging"
...
export const run = Effect.fn("Tui.run")(function* (input: TuiInput) {
  ...
  const shouldPrintLogs = isPrintLogsEnabled()
  const rendererOptions = shouldPrintLogs
    ? { externalOutputMode: "passthrough" as const }
    : {
        screenMode: "split-footer" as const,
        externalOutputMode: "capture-stdout" as const,
        consoleMode: "disabled" as const,
      }
  const originalStderrWrite = shouldPrintLogs ? undefined : process.stderr.write
  if (!shouldPrintLogs) {
        ...
        yield* Effect.addFinalizer(() =>
          Effect.promise(async () => {
            try {
              ...
              logError("Failed to dispose TUI plugins", error)
            }
          }),
        )
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  props.pluginHost
    ...
    .catch((error) => {
      logError("Failed to load TUI plugins", error)
    })
    .finally(() => {
      setReady(true)
    })
  ...
  event.on("installation.update-available", async (evt) => {
    logDebug("installation.update-available", evt)
