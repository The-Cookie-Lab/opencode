...
The assistant footer keeps response metadata compact by default. Click the chevron next to the model and duration to expand token details for the turn. On wide terminals, the sidebar Context section also shows session-level token usage and, when available, a prompt breakdown for input, output, cache, tools, instructions, and template overhead.
