...
const providerErrorEnv = LayerNode.compile(root, [...legacyReplacements, [LLM.node, providerErrorLLM]])
...
const fragmentFailureEnv = LayerNode.compile(root, [...legacyReplacements, [LLM.node, fragmentFailureLLM]])
    ...
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        const starts = (yield* MessageV2.parts(msg.id)).filter(
          (part): part is SessionV1.StepStartPart => part.type === "step-start",
        )
