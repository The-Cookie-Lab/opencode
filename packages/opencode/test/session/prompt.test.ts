...
type PromptLayerOptions = {
  ...
  runtimeFlags?: Partial<RuntimeFlags.Info>
}

const runtimeFlags = (input?: PromptLayerOptions) =>
  RuntimeFlags.layer({ experimentalEventSystem: true, ...input?.runtimeFlags })
...
function promptReplacements(input?: PromptLayerOptions) {
  const flags = runtimeFlags(input)
  ...
  if (input?.processor === "blocking") replacements.push([SessionProcessor.node, blockingProcessor])
  if (input?.planner) replacements.push([ContextPlanner.node, input.planner])
  if (input?.memory) replacements.push([LocalModelServerMemory.node, input.memory])
  return replacements
}

function makePrompt(input?: PromptLayerOptions) {
  return Layer.mergeAll(LayerNode.compile(promptRoot, promptReplacements(input)), runtimeFlags(input))
}

function makeHttp(input?: PromptLayerOptions) {
  const root = LayerNode.group([promptRoot, testLLMServerNode])
  return Layer.mergeAll(LayerNode.compile(root, promptReplacements(input)), runtimeFlags(input))
}
...
const legacyEventSystem = testEffect(makeHttp({ runtimeFlags: { experimentalEventSystem: false } }))
...
const failingMemory = Layer.succeed(
  ...
  LocalModelServerMemory.Service.of({
    ...
    captureMessageParts: () => Effect.fail(new Error("offline")),
...
legacyEventSystem.instance("legacy prompt emits message events without session.next events", () =>
  Effect.gen(function* () {
